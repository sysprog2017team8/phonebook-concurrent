...
static void append(void *arg)
{
    ...
    for (char *i = t_arg->data_begin; i < t_arg->data_end;
        ...
        while(elem->pNext !=NULL) {
            int strlen_lastName = strlen(elem->pNext->lastName);
            int strlen_i = strlen(i);
            int strlen_to_compare =(strlen_lastName < strlen_i)? strlen_i : strlen_lastName;

            if(strncasecmp(elem->pNext->lastName, i, strlen_to_compare) == 0) {
                UNLOCK(elem->lock);
                continue;
            }
